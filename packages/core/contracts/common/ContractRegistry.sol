...
contract ContractRegistry is OwnableUpgradeable {
  ...
  event ContractAddressUpdated(string contractname, bytes32 indexed hash, address contractAddress);

  function initialize() external initializer {
    __Ownable_init();
  }

  function setContractAddress(string memory contractName, address addr) external onlyOwner {

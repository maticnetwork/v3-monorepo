...
contract ValidatorSet is SystemCall {
    ...
    function setInitialValidators() internal {
        ...
        for (uint256 i = 0; i < d.length; i++) {
            validators[span].push(
                Validator({id: i, power: p[i], signer: d[i]})
            );
        }

        for (uint256 i = 0; i < d.length; i++) {
            producers[span].push(Validator({id: i, power: p[i], signer: d[i]}));
        }
    }
    ...
    {
        uint256 span = currentSpanNumber() + 1;
        return (
            spans[span].number,
            spans[span].startBlock,
            spans[span].endBlock
    ...
    function getValidatorsBySpan(uint256 span)
        internal
        view
    ...
    function getProducersBySpan(uint256 span)
        internal
        view
        returns (Validator[] memory)
    {
    ...
    function getSpanByBlock(uint256 number) public view returns (uint256) {
        for (uint256 i = spanNumbers.length; i > 0; i--) {
            ...
            if (
                span.startBlock <= number &&
                span.endBlock != 0 &&
                number <= span.endBlock
            ) {
                return span.number;
            }
        }
    ...
    function getValidatorsTotalStakeBySpan(uint256 span)
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < vals.length; i++) {
            result = result + vals[i].power;
        }
        return result;
    }

    function getProducersTotalStakeBySpan(uint256 span)
        public
        view
        returns (uint256)
    {
    ...
    function getValidatorBySigner(uint256 span, address signer)
        public
        view
        returns (Validator memory result)
    {
        Validator[] memory vals = validators[span];
        for (uint256 i = 0; i < vals.length; i++) {
    ...
    function commitSpan(
        ...
        require(
            spans[span].startBlock <= startBlock,
            "Start block must be greater than current span"
        );
    ...
    function getStakePowerBySigs(
        ...
        for (uint64 i = 0; i < sigs.length; i += 65) {
            ...
            (address signer, ) = dataHash.tryRecover(sigElement);
            ...
            if (isValidator(span, signer) && signer > lastAdd) {
                lastAdd = signer;
                stakePower = stakePower + validator.power;
            }
        }
    ...
    function innerNode(bytes32 left, bytes32 right)
        public
        pure
        returns (bytes32)
    {
